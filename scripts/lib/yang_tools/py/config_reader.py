...
if __name__ == '__main__':
	if len(sys.argv) < 4:
		print("Missing arguements... "+sys.argv[0]+" [cfg file] [tmp path] [out path]")
		sys.exit(1)
	...
	model_list_name = yin_utils.string_to_c_formatted_name("cps_api_category")+"_t"
	model_list = list()
	...
	for l in f:
		...
		if model_cps_name.find("dell-")!=-1:
			model_cps_name = model_cps_name[model_cps_name.find("dell-")+len("dell-"):]
		
		model_list.append(yin_utils.string_to_c_formatted_name("cps_api_cat_"+model_cps_name))
		...
		p = subprocess.Popen (["pyang","-f","yin",model_file],stdout=subprocess.PIPE)
		of = open(yin_file,"w");
