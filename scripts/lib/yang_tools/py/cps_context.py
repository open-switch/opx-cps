...
class context:
    ...
    def __init__(self, args):
        ...
        if len(self.get_arg('debug'))>0:
            context['debug'] = self.__log_level_to_number(self.get_arg('debug'))
                 
        context['temp-setting'] = {}
        context['temp-setting']['remove-files'] = len(self.get_arg('cleanup'))==0
    ...
    def log(self,type,msg, *args):
        __lvl = self.__log_level_to_number(type)
